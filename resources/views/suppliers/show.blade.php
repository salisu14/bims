<x-app-layout>
   ...
   <x-flash />
	...
	<x-card>
        <h1 class="text-blue-500 text-3xl">{{ $supplier->name  }}</h1>
        <ul class="mt-2 text-xl">
            <li class="mb-1">Phone Number: {{ $supplier->phone }}</li>
            <li class="mb-1">Address 1: {{ $supplier->address1 }} </li>

			@isset($supplier->address2)
				<li class="mb-1">Address 2: {{ $supplier->address2 }}</li>
			@endisset

			<li class="mb-1">City/State: {{ $supplier->city->name }}, {{ $supplier->state->name }} state.</li>
			@isset($supplier->website)
				<li class="mb-1">Website: {{ $supplier->website }}</li>
			@endisset

			@isset($supplier->note)
				<li class="mb-1">Note: {{ $supplier->note }}</li>
			@endisset

			@isset($supplier->postal_code)
				<li class="mb-1">Postal Code: {{ $supplier->postal_code }}</li>
			@endisset
        </ul>
	   <div class="mt-4">

			<div class="mr-4 inline">
				<x-anchor-link href="{{ route('suppliers.index') }}">
					Back 
				</x-anchor-link>
			</div>
			...
			@can('deceased_edit')
			<div class="mr-4 inline">
				<x-anchor-link class="" href="{{ route('suppliers.edit', $supplier) }}">
