        ...
        <table class="min-w-full leading-normal table-auto md:table-fixed">
            ...
            @foreach($users as $user)
                <tr>
                    ...
                    <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        <p class="text-gray-900 whitespace-no-wrap">{{ $user->roles->pluck('name') }}</p>
                    </td>

                    <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        <p class="text-gray-900 whitespace-no-wrap">{{ $user->phone_number ?? '0803' }}</p>
                    </td>

                    <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        <p class="text-gray-900 whitespace-no-wrap">{{ $user->store ?? 'not-applicable' }}</p>
                    </td>
                    <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        <div class="mt-4 inline">
                            @can('deceased_edit')
                            <div class="mr-4 inline">
                                <x-anchor-link href="{{ route('users.edit', $user) }}">
                            ...
                            <div class="mr-4 inline">
                                <form class="inline" method="POST" action="{{ route('users.destroy',$user) }}">
                                    ...
                                    <button onclick="return confirm('Are you sure you want delete this user?')" type="submit">
